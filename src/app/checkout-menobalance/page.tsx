...
import {
  ...
  CheckCircle, Banknote, Package, ShieldCheck, Loader2, Moon, Zap
} from 'lucide-react'
...
const ProgressStep = memo(({ number, label, status }: {
  ...
  const getStepStyles = () => {
    switch (status) {
      ...
      case 'active':
        return { circle: 'bg-purple-600 text-white', text: 'text-purple-600 font-bold', line: 'bg-gray-200' }
      default:
  ...
  <div>
    ...
    <div className="flex">
      ...
      <input
        ...
        inputMode={inputMode}
        maxLength={maxLength}
        className={`w-full p-4 md:p-5 bg-gray-50 border-2 ${error ? 'border-red-500' : 'border-gray-200'} ${prefix ? 'rounded-r-xl' : 'rounded-xl'} focus:border-purple-600 focus:bg-white text-lg md:text-xl transition-all touch-manipulation`}
    ...
    <div className="p-5 md:p-6">
      ...
      <div className="flex gap-4 pb-5 border-b border-gray-100">
        ...
        <div className="flex-1">
          <h3 className="font-bold text-gray-900 text-lg">MenoBalance Protocollo</h3>
          <p className="text-base text-gray-500">Kit Giorno + Notte (1 mese)</p>
          <div className="flex items-center gap-2 mt-2">
            <span className="bg-yellow-100 text-yellow-800 text-xs font-bold px-2 py-1 rounded flex items-center gap-1">
              <Zap className="w-3 h-3" /> Giorno
            </span>
            <span className="bg-indigo-100 text-indigo-800 text-xs font-bold px-2 py-1 rounded flex items-center gap-1">
              <Moon className="w-3 h-3" /> Notte
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="py-5 space-y-3 border-b border-gray-100 text-base md:text-lg">
        <div className="flex justify-between">
          <span className="text-gray-500">Prezzo di listino</span>
      ...
      <div className="pt-5">
        <div className="flex justify-between items-center">
          ...
          <span className="text-4xl md:text-5xl font-black text-purple-600">49,99€</span>
        </div>
      ...
      <div className="mt-6 space-y-3">
        ...
        <div className="flex items-center gap-3 text-base text-gray-700 bg-purple-50 p-4 rounded-xl">
...
export default function MenobalanceCheckout() {
  ...
  const validateForm = useCallback(() => {
    ...
    if (!formData.address.trim() || formData.address.trim().length < 10) {
      newErrors.address = 'Inserisci un indirizzo completo con via, civico, CAP e citta'
      isValid = false
    }
  ...
  const handleSubmit = useCallback(async (e: FormEvent) => {
    ...
    try {
      ...
      await fetch('https://network.worldfilia.net/manager/inventory/buy/htf_menopauseenergy.json?api_key=xgM6LBE0CA4EwJ4NTNhPBQ', {
        method: 'POST',
        body: submitData
      })

      window.location.href = `/ty-menobalance?name=${encodeURIComponent(firstName)}`
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b-2 border-gray-200 py-4 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 flex items-center justify-between">
          <Link
            href="/menobalance-system"
            className="flex items-center gap-2 text-gray-600 hover:text-purple-600 transition-colors py-2 touch-manipulation"
          >
          ...
          <div className="grid lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-3 order-2 lg:order-1">
              <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                ...
                {/* Header */}
                <div className="bg-gradient-to-r from-purple-700 to-indigo-700 text-white p-5 md:p-6">
                  <h1 className="text-xl md:text-2xl font-bold flex items-center gap-3">
                    <Truck className="w-7 h-7 text-yellow-400" />
                    Dove spediamo il tuo ordine?
                  </h1>
                  <p className="text-base text-purple-200 mt-2">
                ...
                <form ref={formRef} onSubmit={handleSubmit} className="p-5 md:p-6 space-y-6" noValidate>
                  ...
                  {/* How it works */}
                  <div className="bg-purple-50 border-2 border-purple-200 rounded-2xl p-5 mb-2">
                    <h3 className="font-bold text-gray-900 text-base md:text-lg mb-4 flex items-center gap-2">
                      <Info className="w-5 h-5 text-purple-600" />
                      Come funziona l&apos;ordine:
                    </h3>
                    <ol className="text-base md:text-lg text-gray-700 space-y-3">
                      <li className="flex items-start gap-3">
                        <span className="w-8 h-8 bg-purple-600 text-white rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0">1</span>
                      ...
                      <li className="flex items-start gap-3">
                        <span className="w-8 h-8 bg-purple-600 text-white rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0">3</span>
                  ...
                  <FormInput
                    ...
                    hint="Inserisci via, numero civico, CAP, citta e provincia"
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 active:from-purple-800 active:to-indigo-800 disabled:from-gray-400 disabled:to-gray-400 text-white font-black text-xl md:text-2xl py-5 md:py-6 rounded-2xl shadow-lg transition-all disabled:cursor-not-allowed flex items-center justify-center gap-3 touch-manipulation min-h-[72px]"
                  >
                    {isSubmitting ? (
                      <>
      ...
      <footer className="bg-gray-50 border-t border-gray-200 py-8 mt-8">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <p className="text-sm text-gray-600 mb-3">
            <strong>Health News Network</strong> - P.IVA IT12345678901
          </p>
          ...
          <p className="text-xs text-gray-400 max-w-2xl mx-auto">
            Integratore alimentare. I risultati possono variare. Non e un medicinale. Da usare con dieta equilibrata e stile di vita sano.
            Questo sito non e parte di Facebook o Facebook Inc. FACEBOOK e un marchio registrato di FACEBOOK, Inc.
          </p>
          <p className="text-xs text-gray-400 mt-2">© 2025 Health News Network - Tutti i diritti riservati.</p>
        </div>
      </footer>
    </div>
  )
